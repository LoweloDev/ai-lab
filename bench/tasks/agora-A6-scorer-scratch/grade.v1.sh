#!/usr/bin/env bash
# Grader for agora-A6-scorer-scratch. Workspace dir is $1; the runner wrote the baseline
# git SHA into $1/.bench-baseline. The workspace ships WITHOUT internal/feed/scorer.go
# (and without scorer_test.go); the agent designs RankCandidates from the product brief.
# PASS iff the kept existing test files are untouched and the package suite is green with
# the hidden behavioral property tests copied in (deterministic fake embedding vectors,
# no real model anywhere). The agent's own NEW test files are untracked, so they never
# trip the tamper guard — but they do run, and must pass too.
set -u
...
cp /home/lowelodev/ai-lab/bench/tasks/agora-A6-scorer-scratch/grade_test.go "$tgt"
