#!/usr/bin/env bash
# grade.v2.sh — gehaerteter Grader fuer agora-A4-feed (Staging, Patch-Paket P1 aus audit-report.md 4.1).
...
#     (Existenz + Inhalt); kein git, kein $ws/.bench-baseline (F2, F6, F10). Der cmp-Lauf iteriert ueber die
#     PRISTINE-Dateien, daher faellt auch eine geloeschte/umbenannte batcher_test.go auf (F6, bash-Glob-Loch).
#   * Fremde *_test.go des Modells werden NICHT entfernt (anders als audit-scratch/a4/grade-fixed.sh) —
#     das waere eine Semantikaenderung (n1-workaround); Kaperung wird ueber Pass-Ereignisse neutralisiert.
...
#     Erwartet = Baseline-Tests aus `go test -list` im Pristine (+ Namen aus HIDDEN, hier leer) (F1, F11).
...
PKG=internal/feed
HIDDEN=""                     # A4: kein versteckter Test
